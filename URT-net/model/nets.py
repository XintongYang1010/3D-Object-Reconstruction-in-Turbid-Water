...
class my_model(nn.Module):
    def __init__(self,
        ...
        self.encoder = Encoder(feature_num=en_feature_num, inter_num=en_inter_num, sam_number=sam_number,CHANNEL=CHANNEL)
    ...
    def forward(self, x):
        ...
        # print("nets_28  ","y_1",y_1.shape,"y_2",y_2.shape,"y_3",y_3.shape)
        out_1, out_2, out_3 = self.decoder(y_1, y_2, y_3)
        # print("nets_30  ","out_1",out_1.shape,"out_2",out_2.shape,"out_3",out_3.shape)
...
class Encoder(nn.Module):
    def __init__(self, feature_num, inter_num, sam_number,CHANNEL):
        super(Encoder, self).__init__()

        self.conv_first = nn.Sequential(
            nn.Conv2d(4 * int(CHANNEL) , feature_num, kernel_size=5, stride=1, padding=2, bias=True),
    ...
    def forward(self, x):
        ...
        # print("nets 93 out_feature_1",out_feature_1.shape," out_feature_2 ",out_feature_2.shape," out_feature_3 ",out_feature_3.shape)
...
class Encoder_Level(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("nert 116 out_feature",out_feature.shape)
        ...
        # print("nert 122 out_feature",out_feature.shape)
...
class Decoder_Level(nn.Module):
    ...
    def forward(self, x, feat=True):
        x = self.rdb(x)
        # print("nets 139 x ",x.shape)
        for sam_block in self.sam_blocks:
            x = sam_block(x)
        out = self.conv(x)
        # print("nets 143 out ",out.shape)
        out = F.pixel_shuffle(out, 2)
        # print("nets 145 out ", out.shape)
        if feat:
...
class SAM(nn.Module):
    ...
    def forward(self, x):
        ...
        x_2 = F.interpolate(x, scale_factor=0.5, mode='bilinear', align_corners=True)
        x_4 = F.interpolate(x, scale_factor=0.25, mode='bilinear', align_corners=True)

        if (x.size(-1)==153):
            x_2 = torch.nn.functional.pad(x, [0, 1], value=0)
            x_2 = F.interpolate(x_2, scale_factor=0.5, mode='bilinear',align_corners=True)

            x_4 = torch.nn.functional.pad(x, [0, 3], value=0)
            x_4 = F.interpolate(x_4, scale_factor=0.25, mode='bilinear', align_corners=True)

        if (x.size(-1) == 306):
            x_4= torch.nn.functional.pad(x, [0, 2], value=0)
            x_4 = F.interpolate(x_4, scale_factor=0.25, mode='bilinear',align_corners=True)
        ...
        y_2 = F.interpolate(y_2, scale_factor=2, mode='bilinear',align_corners=True)
        # print("y_4",y_4.shape)

        y_4 = F.interpolate(y_4, scale_factor=4, mode='bilinear',align_corners=True)
        if (y_4.size(-1) == 308):
            y_4=y_4[:,:,:,0:306]
...
class CSAF(nn.Module):
    ...
    def forward(self, x0, x2, x4):
        ...
        # print("x0",x0.shape,"x2",x2.shape,"x4",x4.shape)
        # print("w0",w0.shape,"w2", w2.shape,"w4",  w4.sha pe)
        x = x0 * w0 + x2 * w2 + x4 * w4
        return x
...
class RDB(nn.Module):
    ...
    def forward(self, x):
        ...
        t = self.conv_post(t)
        # print("nets 245  t",t.shape," x ",x.shape)
        return t + x
